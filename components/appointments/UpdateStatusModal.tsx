...
const ALL_STATUSES: { status: AppointmentStatus; label: string; desc: string }[] = [
  { status: 'CONFIRMED', label: 'Confirmed', desc: 'Client booked or confirmed their attendance' },
  { status: 'COMPLETED', label: 'Completed', desc: 'Service successfully completed' },
  { status: 'PENDING', label: 'Pending', desc: 'Pending confirmation with the client' },
  { status: 'NO_SHOW', label: 'No Show', desc: 'The client did not show up for the appointment' },
  { status: 'CANCELLED', label: 'Cancelled', desc: 'Appointment cancelled by client or barber shop' },
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
          <div>
            <h3 className="text-base font-bold text-white">Update Appointment Status</h3>
            <p className="text-xs text-zinc-400">
              Appointment #{appointment.id} • {appointment.client?.name || 'Client'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2">
              {ALL_STATUSES.map((item) => (
                <label
                  key={item.status}
                  className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${status === item.status
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Ej. 'Client rescheduled', 'Made cash payment'..."
